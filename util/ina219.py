...
Source: https://github.com/chrisb2/pi_ina219
Supports the GreatFET using the I2C bus.
"""
...
class INA219:
    ...
    def __init__(self, gf, shunt_ohms, max_expected_amps=None,
    ...
    def configure(self, voltage_range=RANGE_32V, gain=GAIN_AUTO,
        ...
        #print('gain set to %.2fV' % self.__GAIN_VOLTS[self._gain])
    ...
    def _calibrate(self, bus_volts_max, shunt_volts_max,
        ...
        # print("max possible current: %.3fA" %
        #                  max_possible_amps)

        self._current_lsb = \
            self._determine_current_lsb(max_expected_amps, max_possible_amps)
        # print("current LSB: %.3e A/bit" % self._current_lsb)

        self._power_lsb = self._current_lsb * 20
        # print("power LSB: %.3e W/bit" % self._power_lsb)
        ...
        # print("max shunt voltage before overflow: %.4fmV" %
    ...
    def _determine_current_lsb(self, max_expected_amps, max_possible_amps):
        if max_expected_amps is not None:
            ...
            # print("max expected current: %.3fA" %
            #                  max_expected_amps)
    ...
    def _configuration_register(self, register_value):
        # print("configuration: 0x%04x" % register_value)
    ...
    def _read_gain(self):
        ...
        # print("gain is currently: %.2fV" % self.__GAIN_VOLTS[gain])
        return gain
    ...
    def _configure_gain(self, gain):
        ...
        self._gain = gain
        # print("gain set to: %.2fV" % self.__GAIN_VOLTS[gain])

    def _calibration_register(self, register_value):
        # print("calibration: 0x%04x" % register_value)
    ...
    def __write_register(self, register, register_value):
        ...
        self._i2c.write(self._address, bytes([register]) + register_bytes)

    def __read_register(self, register, negative_value_supported=False):

        self._i2c.write(self._address, bytes([register]))
        resp = self._i2c.read(self._address, 2)

        if negative_value_supported:
            register_value = (((resp[0] << 8) | resp[1]) & 0x7FFF) * (-1 if resp[0] & 0x80 else 1)
        else:
            register_value = (resp[0] << 8) | resp[1]
        # print(
        #     "read register 0x%02x: 0x%04x 0b%s" %
        #     (register, register_value,
        #      self.__binary_as_string((resp[0] << 8) | resp[1])))
        return register_value

    def __to_bytes(self, register_value):
        return bytes([(register_value >> 8) & 0xFF, register_value & 0xFF])
